...
class ActivityMaker extends LifecycleMaker {
    ...
    private void brewOnPostCreateMethod(TypeSpec.Builder builder) {
        ...
        codeBlock.addStatement("super.onPostCreate(savedInstanceState)");
    ...
    private void brewOnDestroyMethod(TypeSpec.Builder builder) {
        final CodeBlock.Builder codeBlock = CodeBlock.builder();
        codeBlock.addStatement("$L.clear()", M_ON_CLICK);
        codeBlock.addStatement("$L.clear()", M_ON_ACTION_CLICK);
        codeBlock.addStatement("super.onDestroy()");
    ...
    private void brewFindAllViewsMethod(TypeSpec.Builder builder) {
        ...
        for (final Map.Entry<Element, InjectView> entry : mInjectView.entrySet()) {
            codeBlock.addStatement("$L.$L = $T.findById(this, $L)", M_DELEGATE, entry.getKey().getSimpleName(),
                    ClassName.get("droidkit.view", "Views"), entry.getValue().value());
        }
